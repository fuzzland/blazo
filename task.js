...
function handleBuildContracts(result_json) {
  if (!result_json) {
    return false;
  }
  ...
  result_json.forEach((item) => {
    for (let fn in item["bytecode"]) {
      // filter foundry lib
      if (fn.includes("lib/forge-std")) {
        continue;
      }
      for (let contract in item["bytecode"][fn]) {
        if (
          contract === "FuzzLand" ||
          contract.startsWith("__ScribbleUtilsLib")
        ) {
          continue;
        }
        let idx = fn + ":" + contract;
        if (knownContracts.includes(idx)) {
          continue;
        }
        knownContracts.push(idx);
        contractList.push({
          file: fn,
          name: contract,
          bytecode: item["bytecode"][fn][contract],
          abi: item["abi"][fn][contract],
...
async function createOffchain(projectPath, projectType, compilerVersion) {
  ...
  output.on("close", async () => {
    ...
    try {
      ...
      formData.append("file", fs.createReadStream(path.join("output.zip")));
      ...
      const {
        ...
        formData,
        {
          params: {
            ...
            needs:
              "sourcemap,bytecode,abi,sources,invariants,compiler_args,ast",
          },
          ...
          .then((response) => {
            ...
            if (data.status === "done") {
              ...
              return new Promise((resolve) => {
                setTimeout(
                  () => resolve(handleLoadBuilderResult(attempts + 1)),
                  1000
                );
              });
            }
      ...
      await new Promise((resolve) => {
        process.stdin.once("data", (chunk) => {
          offchainConfig = fs.readFileSync("offchain_config.json", "utf8");
          resolve();
        });
      });
      handleBuildContracts(result_json);
...
async function createOnchain(type, contractAddress, chain, blockNumber) {
  ...
  try {
    const response = await axios.post(`${BLAZ_BASE_URL}/task/onchain`, data, {
      headers: {
        Authorization: BLAZ_API_KEY,
      },
    });
    if (response.data.status === "success") {
      ...
      logger.error("Failed to create onchain task:", response.data.message);
    }
  } catch (error) {
